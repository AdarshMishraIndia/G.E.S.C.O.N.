...
# Set desired video resolution to 720p
VIDEO_WIDTH = 1280
VIDEO_HEIGHT = 720
...
interaction_box_margin = 120  # Margin for smaller green box than yellow box
gesture_box_margin = 100     # Margin for bigger yellow box than green box
...
# # Old gesture box dimensions
# gesture_x1 = max(0, x1)
# gesture_y1 = max(0, y1)
# gesture_x2 = min(VIDEO_WIDTH, x2)
# gesture_y2 = min(VIDEO_HEIGHT, y2)
...
while True:
    ...
    if hands:
        ...
        if fingers[1] and fingers[2] and not fingers[3] and not fingers[4]:  # Index and middle extended
            ...
            elif time.time() - gesture_start_time > 0.5:  # If extended for more than 1 second
            ...
            if gesture_start_time is not None and time.time() - gesture_start_time <= 0.5:
        ...
        elif fingers.count(True) == 3 and fingers[1] and fingers[2] and fingers[3]:  # Three fingers for right-click
        ...
        elif fingers.count(True) == 5:  # Five fingers for recent apps toggleTurkey Singham Bollywood
            if recent_apps_open:
       ...
       # Closed fist for starting drag
        if fingers.count(True) == 0:  
            if not dragging:
                pyautogui.mouseDown()
                dragging = True
        else:  # Any other gesture resets dragging
            if dragging:  # Only reset if dragging is active
                pyautogui.mouseUp()
                dragging = False
